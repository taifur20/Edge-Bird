/***********************************************************************
   ESP32-S3 Bird Environment Logger
   -------------------------------------------------------------
...
String getTimestamp() {
  ...
  char buf[32];
  snprintf(buf, sizeof(buf), "%02lu:%02lu:%02lu", hrs % 24, mins % 60, sec % 60);
  return String(buf);
}
...
// Run Edge Impulse model and return best label
...
void setup() {
  ...
  if (f && f.size() == 0)
    f.println("Timestamp,Temp(C),Humidity(%),Pressure(hPa),MagX(uT),MagY(uT),MagZ(uT),Battery(V),Battery(%),Bird");
  f.close();
...
void loop() {
  ...
  if (df) {
    df.print(getTimestamp()); df.print(",");
